...
use vulkanalia::vk::{ComponentSwizzle, KhrSwapchainExtension};
...
fn main() -> Result<()> {
    ...
    event_loop.run(move |event, _, control_flow| {
        *control_flow = ControlFlow::Poll;
        match event {
            Event::MainEventsCleared if !destroy => unsafe { app.render(&window) }.unwrap(),
            Event::WindowEvent {
...
unsafe fn create_instance(window: &Window, entry: &Entry) -> Result<Instance> {
    let app_info = vk::ApplicationInfo::builder()
        .application_name(b"Vulkan Intro\0")
    ...
    let flags = if cfg!(target_os = "macos") && entry.version()? >= PORTABILITY_MACOS_VERSION {
        info!("Enabling exensions for macOS");
        extensions.push(
    ...
    let mut dbg_info = vk::DebugUtilsMessengerCreateInfoEXT::builder()
        .message_severity(vk::DebugUtilsMessageSeverityFlagsEXT::all())
        .message_type(vk::DebugUtilsMessageTypeFlagsEXT::all())
        .user_callback(Some(debug_callback));

    if VALIDATION_ENABLED {
        info = info.push_next(&mut dbg_info);
    }
...
unsafe fn create_swapchain(
    ...
    let mut image_count = support.capabilities.min_image_count;
...
unsafe fn pick_physical_device(instance: &Instance, data: &mut AppData) -> Result<()> {
    ...
    Err(anyhow!("Failed to get suitable device"))
}
...
unsafe fn check_physical_device_extensions(
    ...
    if DEVICE_EXTENSIONS.iter().all(|e| extensions.contains(e)) {
        ...
        Err(anyhow!(SuitabilityError("Missing device extensions")))
    }
}
